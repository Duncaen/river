...
/// The cursor movement since the last jitter reset or hide timeout
jitter: u32 = 0,
...
pub fn init(self: *Self, seat: *Seat) !void {
    ...
    try self.hide_cursor_timer.timerUpdate(self.getHideTimeout());
...
fn getHideTimeout(self: *Self) u31 {
    if (self.pressed_count > 0)
        return 0;
    return server.config.cursor_hide_timeout;
}
...
pub fn unhide(self: *Self) void {
    self.jitter = 0;
    self.hide_cursor_timer.timerUpdate(self.getHideTimeout()) catch {
        log.err("failed to update cursor hide timeout", .{});
    };
    if (!self.hidden)
        return;
    const image = if (self.image != .unknown) self.image else .left_ptr;
    self.image = .unknown;
    self.setImage(image);
    self.hidden = false;
}
...
fn processMotion(self: *Self, device: *wlr.InputDevice, time: u32, delta_x: f64, delta_y: f64, unaccel_dx: f64, unaccel_dy: f64) void {
    const jitter_limit = if (self.hidden)
        server.config.cursor_unhide_jitter
    else
        server.config.cursor_hide_jitter;
    self.jitter += @floatToInt(u32, @fabs(delta_x) + @fabs(delta_y));
    if (self.jitter > jitter_limit) {
        self.unhide();
    } else if (self.hidden) {
        // If the cursor is hidden and the jitter limit is not reached, ignore the motion.
        return;
    }
